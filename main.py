...
for i in range(1, loadbarwidth + 1):
    ...
    strbarwidth = '[{}{}] - {}\r'.format(
        (i * '✓'),
        ((loadbarwidth - i) * '-'),
        (('{:0.2f}'.format(((i) * (100 / loadbarwidth))) + '% Lączenie'))
    )
...
def save_to_file(n):
    save_path = 'wynik/{}_lat_{}_lon_{}.json'.format(time.strftime("%Y-%m-%d_%Hx%M"), lat1, lon1)
    try:
...
def start():
    while True:
        ...
        wspol = (x + "," + y)
        url = 'http://beta.btsearch.pl/?dataSource=locations&network=26001&standards=&bands=&center=' + (
            wspol) + '&zoom=20'
        print(url)
